import {
  getfindByLetterDB,
  getfindWordDB,
} from "../../services/dictionary/wordServices.js";
  ...
  try {
    ...
    if (data.original !== "") {
      ...
      if (result) {
        ...
        return res.status(200).json({ message: "Word not found", data: data });
      }
    } else {
      return res
        .status(400)
        .json({ error: "You must enter a value in the search" });
    }
  } catch (error) {
...
const getfindByLetterController = async (req, res) => {
  try {
    ...
    if (data.original !== "") {
      ...
      return res
        .status(400)
        .json({ error: "You must enter a letter in the search" });
    }
  } catch (error) {
    res.status(400).json({ error: error.message }); // Send an error response
  }
